...
if __name__ == "__main__":
    ...
    train_data_map = {}
    train_size_map = {}
    for partition_id, partition_size, object_id, ip in train_res:
        if ip not in train_data_map:
            train_data_map[ip] = []
            train_size_map[ip] = []
        train_data_map[ip].append(object_id)
        train_size_map[ip].append(partition_size)
    for node, data in train_size_map.items():
        print("Node {} has {} subpartitions and {} train records".format(node, len(data), sum(data)))
    save_end = time.time()
    ...
    test_rdd = test_rdd.repartition(args.num_nodes * args.cores)
    test_rdd.cache()
    ...
    test_object_ids = [[plasma.ObjectID.from_random() for j in range(5)]
    ...
    test_data_map = {}
    test_size_map = {}
    for partition_id, partition_size, object_id, ip in test_res:
        if ip not in test_data_map:
            test_data_map[ip] = []
            test_size_map[ip] = []
        test_data_map[ip].append(object_id)
        test_size_map[ip].append(partition_size)
    for node, data in test_size_map.items():
        print("Node {} has {} subpartitions and {} test records".format(node, len(data), sum(data)))
    ...
    # print(res)
    # print("Test data deleted on plasma")
    estimator.shutdown()
