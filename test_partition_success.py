...
def map_to_id(map_broadcast, category):
    feature_map = map_broadcast.value
    if category in feature_map:
        return feature_map[category]
    else:
        return 0
    ...
    def save_func(index, iterator):
        ...
        with FileLock("data.lock"):
            if not os.path.isdir(path):
                os.mkdir(path)
        data = list(iterator)
        X_int = np.concatenate([np.array([row[1]], dtype=np.int32) for row in data])
        X_cat = np.concatenate([np.array([row[2]], dtype=np.int32) for row in data])
        y = np.concatenate([np.array([row[0]], dtype=np.int32) for row in data])
...
def data_creator(config, data_path):
    from zoo.orca.data.utils import get_size, index_data
    from torch.utils.data import Dataset, DataLoader
    print("Number of files on this worker: ", len(os.listdir(data_path)))
    X_int_list = []
    X_cat_list = []
    y_list = []
    start = time.time()
    for file in os.listdir(data_path):
        with np.load(os.path.join(data_path, file)) as data:
            X_int_list.append(data["X_int"])  # continuous  feature
            X_cat_list.append(data["X_cat"])  # categorical feature
...
def train_data_creator(config):
    return data_creator(config, config["train_data_path"])


def test_data_creator(config):
    return data_creator(config, config["test_data_path"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--driver_path", type=str, default="",
                        help="The data path on the driver")
    parser.add_argument("--hdfs_path", type=str, default=None,
                        help="HDFS path for raw data")
    ...
    with np.load(args.driver_path + "train_fea_count.npz") as data:
        ln_emb = data["counts"]

    # tmp path for preprocessed training and test data on executors
    train_tmp_dir = os.path.join("/tmp", str(uuid.uuid4()))
    test_tmp_dir = os.path.join("/tmp", str(uuid.uuid4()))
    print("Train data files on executor path: ", train_tmp_dir)
    print("Test data files on executor path: ", test_tmp_dir)
    ...
    sc = init_orca_context(cluster_mode=args.cluster_mode, cores=args.cores,
                           ...
                           conf={"spark.network.timeout": "10000000",
                                 ...
                                 "spark.sql.broadcastTimeout": "14400", "spark.sql.shuffle.partitions": "2000",
    ...
    train_rdd = train_rdd.repartition(args.num_nodes * args.cores)
    train_rdd.cache()
        ...
        def f(index, iterator):
            ...
            for record in iterator:
                if len(buffer) == part_size:
                    ...
                    object_id = client.put(buffer, target_id)
    ...
    res = train_rdd.mapPartitionsWithIndex(put_to_plasma(train_object_ids, object_store_address)).collect()
    partition_map = {}
    for partition_id, data_size, object_id, node_ip in res:
        if node_ip not in partition_map:
            partition_map[node_ip] = []
        partition_map[node_ip].append((object_id, data_size))
    client = plasma.connect(object_store_address)
    for node, data in partition_map.items():
        print("Node {} has {} subpartitions and {} records".format(node, len(data), sum([x[1] for x in data])))
    #time.sleep(3000)

    # save_start = time.time()
    # print("Saving train data files on executor path: ", train_tmp_dir)
    # train_rdd.mapPartitionsWithIndex(save(train_tmp_dir)).collect()
    # save_end = time.time()
    ...
    # config["train_data_path"] = train_tmp_dir
    # config["test_data_path"] = test_tmp_dir
    #
    ...
    # res = sc.range(0, args.num_nodes, numSlices=args.num_nodes).mapPartitions(remove(train_tmp_dir)).collect()
    # print(res)
    # print("Train data deleted on executors")
    #
    # # Validation
    # test_start = time.time()
    # test_df = spark.read.schema(schema).option("sep", "\t").csv(args.hdfs_path + "day_23")
    ...
    # print("Saving test data files on executor path: ", test_tmp_dir)
    # test_rdd.mapPartitionsWithIndex(save(test_tmp_dir)).collect()
    ...
    # res = sc.range(0, args.num_nodes, numSlices=args.num_nodes).mapPartitions(remove(test_tmp_dir)).collect()
    # print(res)
    # print("Test data deleted on executors")
    # estimator.shutdown()
    #
    # end_time = time.time()
    # print("Train stats: ", stats)
