...
def _main():
    ...
    if args.mode == 'generate_models':
        ...
        spark.conf.set('spark.sql.shuffle.partitions', 336)
    ...
    args.low_mem = False
    convert_output_folder = "/mnt/disk1/train/"
    ...
    df = load_raw(spark, args.input_folder, args.day_range)
    convert_output_folder = "/mnt/disk1/test/"
    if args.mode == 'transform':
        time_start = time()
        #spark.conf.set('spark.sql.shuffle.partitions', args.days * args.apply_shuffle_parallel_per_day*8)
        spark.conf.set('spark.sql.shuffle.partitions', 336)
    ...
    df = load_raw(spark, args.input_folder, args.day_range)
    convert_output_folder = "/mnt/disk1/validation/"
    if args.mode == 'transform':
        time_start = time()
        #spark.conf.set('spark.sql.shuffle.partitions', args.days * args.apply_shuffle_parallel_per_day*8)
        spark.conf.set('spark.sql.shuffle.partitions', 336)
