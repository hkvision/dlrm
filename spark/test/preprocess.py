...
def _main():
    ...
    sc = init_orca_context(cluster_mode="yarn", num_nodes=8, cores=36, memory="125g", driver_memory="36g",
                           conf={"spark.network.timeout": "10000000", "spark.sql.hive.filesourcePartitionFileCacheSize": "4096000000",
    ...
    time1 = time()
    args = _parse_args()
    print("frequency limit", args.frequency_limit)
    ...
    if args.mode == 'transform':
        ...
        with _timed('transform'):
            ...
            print(rdd.take(5))
            total = rdd.count()
            print("Preprocessed count: ", total)
        time_end = time()
        print(F"Transform train data consuming time:{time_end-time_start}")
